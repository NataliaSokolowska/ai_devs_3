...
export async function POST() {
  try {
    ...
    const questionMatch = questionPage.match(
      /<p id="human-question">.*?<br\s*\/?>\s*(.*?)<\/p>/,
    );
    ...
    const openAiResponse = await connectWithOpenAi(
      question,
      'Return only the correct answer. Be concise. If the question includes "rok", return only the year as a number.',
    );
    ...
    if (openAiResponse.ok) {
      ...
      console.error(
        openAiResponse.error || "Failed to fetch answer from OpenAI",
      );
    }
    ...
    const fileUrl = urlMatch
      ? `${process.env.QUESTION_URL_01}${urlMatch[1]}`
      : null;
