...
export async function POST() {
  try {
    const questionResponse = await fetch(
      process.env.QUESTION_URL_02 as string,
      {
        method: "POST",
        headers: {
    ...
    if (!questionResponse.ok) {
      console.error(
        `Failed to fetch question. Status: ${questionResponse.status}`,
      );
      return NextResponse.json({
        error: `Failed to fetch question. Status: ${questionResponse.status}`,
      });
    }
    ...
    if (question.includes("capital of Poland")) {
      ...
      const openAiResponse = await connectWithOpenAi(
        question,
        "You are a helpful assistant. Answer concisely and accurately. Always respond in English, even if the question is asked in another language.",
      );

      if (!openAiResponse.ok) {
        console.error(
          openAiResponse.error ||
            `Failed to fetch OpenAI response. Status: ${openAiResponse.status}`,
        );
        return NextResponse.json({
          error:
            openAiResponse.error ||
    ...
    if (!verificationResponse.ok) {
      console.error(
        `Failed to send answer. Status: ${verificationResponse.status}`,
      );
      return NextResponse.json({
        error: `Failed to send answer. Status: ${verificationResponse.status}`,
      });
    }
